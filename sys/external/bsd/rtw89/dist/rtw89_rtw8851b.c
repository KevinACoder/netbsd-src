...
/*
 * Lab (RK3568): upstream Linux moved the 8851B firmware format to 1
 * (`rtw8851b_fw-1.bin`, i.e. the base image plus a newer section set in a
 * trailer) and its linux-firmware base file is now the legacy image.  With
 * FW_FORMAT_MAX 0 this port loaded the legacy image, its firmware download
 * stopped after 7 H2C frames (~14 KB) instead of the ~284 KB the working
 * Linux lane sends, and the WCPU never reported ready ("fw unexpected status
 * 1" / chip info setup failed -60).  Linux v7.3's rtw8851b.c has
 * RTW8851B_FW_FORMAT_MAX 1; this mirrors that, and requires
 * rtw8851b_fw-1.bin to be staged in /boot/firmware/rtw89/.
 */
