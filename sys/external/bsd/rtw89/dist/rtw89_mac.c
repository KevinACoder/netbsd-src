...
#ifdef RTW89_LAB_USB
	rtw89_usb_lab_tr('d', (h2c_class << 8) | h2c_func, h2c_return,
	    h2c_seq);
#endif
	rtw89_debug(rtwdev, RTW89_DBG_FW,
...
#ifdef RTW89_LAB_USB
	rtw89_usb_lab_tr('S', (reason << 8) | status, 0, 0);
#endif
...
/*
 * Lab (RK3568): upstream Linux wraps the exchange above in a retry loop
 * (CPU_IO_RX_RETRY_CNT, mac.h) because the firmware can take longer than
 * RTW89_C2H_TIMEOUT_USB (4 ms) to answer the WOW_CPUIO_RX_CTRL H2C over USB.
 * The v7.0 snapshot this port is based on predates that loop, so the first
 * (and only) attempt timed out, rtw89_fw_msg_reg() returned -ETIMEDOUT and
 * the whole attach died with "c2h reg timeout" / "chip info setup failed:
 * -60" - measured on the board: 31 reads of R_AX_C2HREG_CTRL over 3.9 ms,
 * while the ground-truth Linux run reads the same register 48 times and then
 * sees the acknowledgement.  Same fix as upstream, verbatim.
 */
