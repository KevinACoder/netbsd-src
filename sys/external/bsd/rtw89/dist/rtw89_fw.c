...
/*
 * C2H control-register poll with upstream poll_timeout_us_atomic() accounting:
 * only the inter-op delay is charged against the budget, never the polled op
 * itself -- upstream assumes an MMIO read, but on USB one rtw89_read8() is an
 * EP0 vendor transfer of ~90-125 us.  The LinuxKPI read_poll_timeout_atomic()
 * uses a microtime() wall-clock deadline instead, which caps the nominal
 * RTW89_C2H_TIMEOUT_USB (4 ms) at ~31-45 polls; the firmware answers the
 * GET_FEATURE C2H handshake in 5.2-6.1 ms (~41-48 polls), so every USB attach
 * died with ETIMEDOUT in rtw89_fw_read_c2h_reg().  Proven against the Linux
 * ground truth by the rtw89_core.lab_c2h_wall A/B (linux-upstream fw.c).
 */
static int
rtw89_poll_c2h_ctrl_ready(struct rtw89_dev *rtwdev, u32 reg, u32 timeout_us,
			  u8 *val)
{
	s64 left_ns = (s64)timeout_us * 1000;	/* usec -> nsec */
	bool expired;

	for (;;) {
		expired = timeout_us != 0 && left_ns < 0;
				 ...
				 struct rtw89_mac_c2h_info *info)
{
	...
	ret = rtw89_poll_c2h_ctrl_ready(rtwdev, chip->c2h_ctrl_reg, timeout,
					&val);
	if (ret) {
