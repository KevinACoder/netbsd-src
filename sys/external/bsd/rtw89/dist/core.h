...
#ifdef RTW89_LAB_USB
/* RK3568 lab: trace-ring hook implemented by the native usbdi transport
 * (sys/dev/rtw89/rtw89_usb.c); used by the cond/C2H probes below. */
void rtw89_usb_lab_tr(u8 tag, u32 x, u32 v, u32 len);
#endif

/* RK3568 lab diagnostic (2026-09-15): the FW executes FW_OFLD-class H2Cs
 * with >100ms latency on this lane (REC_ACK in ~ms, DONE_ACK often later);
 * upstream's 100ms budget turns that into -60 scan-offload failures.
 * 10x headroom while the latency source is hunted. */
#define RTW89_WAIT_FOR_COND_TIMEOUT msecs_to_jiffies(1000)
