...
/*
 * Lab (RK3568): the native usbdi transport (sys/dev/rtw89/rtw89_usb.c) uses
 * only the protocol constants above; the Linux usb_device/URB bookkeeping
 * below has no LinuxKPI counterpart on FreeBSD (no struct usb_anchor, no
 * usb_get_dev/usb_put_dev, no usb_endpoint_num), so it is compiled out
 * rather than stubbed.  contrib usb.c is not built on this lane either.
 */
#else
