...
/*
 * Lab (RK3568): on FreeBSD the native usbdi transport
 * (sys/dev/rtw89/rtw89_usb.c) owns the driver registration and carries its
 * own copy of the chip data below, so nothing in this translation unit
 * references these objects.  They are kept because they are the reference
 * for that copy, and the resulting warnings are silenced here.
 */
#if defined(__FreeBSD__) && defined(RTW89_LAB_USB)
...
#if defined(__FreeBSD__) && defined(RTW89_LAB_USB)
/*
 * Lab (RK3568): the native usbdi transport (sys/dev/rtw89/rtw89_usb.c) owns
 * the ID table and the driver registration.  FreeBSD builds one module out
 * of the core, the chip tables and the transport, so a second usb_driver
 * registration here would be redundant - and unbuildable anyway, because
 * the LinuxKPI has no interface-class match macro (USB_DEVICE_AND_
 * INTERFACE_INFO) and no usb_driver plumbing to go with it.
 */
#else
