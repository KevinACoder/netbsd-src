			     ...
			     struct rtw89_wait_response *prep, int err)
{
	unsigned long time_left;
#ifdef RTW89_LAB_USB
	unsigned int lab_cond = atomic_read(&wait->cond);

	rtw89_usb_lab_tr('w', lab_cond, err, 0);
#endif
	if (IS_ERR(prep)) {
...
#ifdef RTW89_LAB_USB
			rtw89_usb_lab_tr('y', lab_cond, err, 0);
#endif
			return err;
		}
...
#ifdef RTW89_LAB_USB
	rtw89_usb_lab_tr('y', lab_cond, err, wait->data.err);
#endif
	if (err)
		return err;
...
#ifdef RTW89_LAB_USB
	rtw89_usb_lab_tr('k', cond, data->err, cur == cond);
#endif
	if (cur != cond)
		return;
